...
- **Contributeurs précieux** : Louis Vandevoorde (Musique), Augustin Louis (Scénario et Graphisme), Emka (Graphisme), Romain(aspect éducatif).

### Répartition des tâches :
Nous nous sommes répartis les tâches naturellement selon nos forces : Christophe s'est concentré sur l'algorithmique pure, sur l'affichage et la logique de jeu tandis qu'Eva a géré toute la cohérence du projet et le lien avec le scénario et les sujets éducatifs abordés.

#### Temps passé (Estimation) :

1. Développeurs : 1 mois complet, avec des sessions quotidiennes allant de 2h à 7h de codage.

2. Artistes : Une semaine pour Emka, et environ 10h pour Augustin et Louis.
...
Plutôt que d'utiliser du SQL, nous avons créé un système de gestion de données complexe en Python pur. Les dictionnaires imbriqués dans datacenter.py permettent de gérer les récompenses (reward), les types de biomes et les dialogues dynamiques. Ils gérons également le placement adaptatif des objets pour nous assurer que l'expérience reste fluide peu importe la taille de la fenêtre.

### Analyse critique et gestion des threads :
Le passage à une interface graphique réactive avec Flet nous a confrontés à la complexité de l'asynchronisme. Nous avons identifié des comportements mineurs liés à la gestion du Main Thread (le fil d'exécution principal) et du rafraîchissement du buffer graphique.

Par exemple, l'encapsulation profonde de certaines fonctions de dialogue peut parfois freiner l'actualisation visuelle. Ces défis techniques nous ont poussés à implémenter des mécanismes de forçage de rendu (comme le redimensionnement de fenêtre) pour garantir la fluidité. Cette expérience a été cruciale pour comprendre comment une interface logicielle communique avec les ressources processeur de la machine.

## 🤖 5. Nature et usage de l'Intelligence Artificielle

**Originalité du projet :**
Leafy Life est une création 100% originale, développée depuis une page blanche, sans réutilisation de code externe.

**Recours à l'IA :**
Nous avons eu recours à Claude (modèle *claude-sonnet-4-6*) de manière ponctuelle et raisonnée, dans trois contextes distincts :

- **Débogage avancé** : nous avons sollicité l'IA pour comprendre des comportements non évidents liés à l'asynchronisme et à la gestion des threads sous Flet (problèmes sans message d'erreur explicite).
- **Explication de concepts** : nous nous en sommes servis comme ressource pédagogique pour approfondir des notions comme le Main Thread ou le rafraîchissement du buffer graphique.
- **Accompagnement administratif** : nous avons également utilisé l'IA pour clarifier les modalités d'inscription et de rendu du concours.

Lorsque du code était généré, il concernait uniquement des tâches répétitives, et nous le relisions et l'adaptions systématiquement avant intégration. Nous estimons la proportion de code influencée par l'IA à **moins de 20% du projet global**.

## 🚀 6. Ouverture

Leafy Life s'inscrit dans une démarche durable de création. À l'avenir, nous pourrions décliner ce moteur pour d'autres enjeux sociétaux (santé, social, etc.). Pour Leafy Life lui-même, nous envisageons d'ajouter de nouveaux biomes, de développer l'histoire et d'enrichir le contenu (nouveaux items, compétences et Leafs).